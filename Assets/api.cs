...
public class ChemicalAssistant2 : MonoBehaviour
{
    ...
    public bool enableControllerInput = true; // 是否启用手柄输入
    public bool enableKeyboardInput = true; // 是否启用键盘输入作为备用

    void Start()
    {
        // 输出手柄输入配置信息
        Debug.Log("手柄输入配置已初始化:");
        Debug.Log("启用手柄输入: " + enableControllerInput);
        Debug.Log("启用键盘输入: " + enableKeyboardInput);
        Debug.Log("支持的手柄按钮: A, B, X, Y");
    }
    ...
    void Update()
    {
        ...
        if (enableControllerInput)
        {
            ...
            // 检测手柄A按钮 (joystick button 0) 作为备用
        ...
        // 保留键盘A键作为备用测试方式
        if (enableKeyboardInput && Input.GetKeyDown(KeyCode.A))
        {
            Debug.Log("键盘A键被按下");
            StartCoroutine(TestCoroutine());
        }
    ...
    {
        Debug.Log("开始分析化学方程式: " + chemicalFormula);
        ...
        // 读取图片文件
        string imagePath = Application.dataPath + "/zn.jpg";
